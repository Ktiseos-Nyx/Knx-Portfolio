...
export default function ModelsPage() {
  const { status: ageStatus, verify, deny } = useAgeVerification();
  const [typeFilter, setTypeFilter] = useState<string>("All");
  const [baseFilter, setBaseFilter] = useState<string>("All");
  const [showNsfw, setShowNsfw] = useState(false);

  const filteredModels = SAMPLE_MODELS.filter((m) => {
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Browse our collection of open-source AI art models. Content from
          Civitai will load dynamically in the future.
        </p>
      </div>
